# Operator Research Queue

> **Purpose:** Track which operators have been profiled and which are next
> **Updated by:** Operator Scout agent (every 6 hours)
> **Scoring:** LINX ICP Fit Score 0-100

---

## Scoring Framework

```
LINX Operator ICP Fit Score (0-100):
- Asset proximity Tier 1-3 (20 pts): owner=20, owner+managed=15, curated=10
- Revenue band $1M-$50M (15 pts): sweet spot $5-25M=15, <$1M=5, >$100M=5
- Referral dependency >50% (20 pts): >80%=20, 50-80%=15, 30-50%=10, <30%=5
- Commission arbitrage (15 pts): pays >15% now=15, 10-15%=10, <10%=5
- Brand quality / luxury positioning (10 pts)
- Technology readiness (10 pts): has API/modern site=10, basic site=5, none=0
- Approachability (10 pts): Stein direct=10, 1-degree=7, cold=3
```

## Profiled (Complete)

| # | Operator | Vertical | ICP Fit | Status | Profile Location |
|---|----------|----------|---------|--------|-----------------|
| 1 | Pelorus | Expedition/Yacht | **85** | PROFILED | `profiles/pelorus.md` |
| 2 | Cookson Adventures | Expedition | **84** | PROFILED | `profiles/cookson-adventures.md` |
| 3 | Great Plains Conservation | Safari/Lodge | **83** | PROFILED | `profiles/great-plains-conservation.md` |
| 4 | White Desert | Adventure/Expedition | **82** | PROFILED | `profiles/white-desert.md` |
| 5 | Singita | Safari/Lodge | **82** | PROFILED | `profiles/singita.md` |
| 6 | Angama | Safari/Lodge | **81** | PROFILED | `profiles/angama.md` |
| 7 | Natural Habitat Adventures | Adventure/Wildlife | **81** | PROFILED | `profiles/natural-habitat-adventures.md` |
| 8 | NIHI Sumba | Luxury Resort | **79** | PROFILED | `profiles/nihi-sumba.md` |
| 9 | Butterfield & Robinson | Adventure/Active | **77** | PROFILED | `profiles/butterfield-and-robinson.md` |
| 10 | Wilderness Safaris | Safari/Lodge | 76 | PROFILED | `profiles/wilderness-safaris.md` |
| 11 | Cecil Wright & Partners | Yacht Charter | 74 | PROFILED | `profiles/cecil-wright-and-partners.md` |
| 12 | Burgess Yachts | Yacht Charter | 72 | PROFILED | `profiles/burgess-yachts.md` |
| 13 | Y.CO | Yacht Charter | 71 | PROFILED | `profiles/y-co.md` |
| 14 | Aman Resorts | Luxury Resort | 70 | PROFILED | `profiles/aman-resorts.md` |
| 15 | Edmiston & Company | Yacht Charter | 69 | PROFILED | `profiles/edmiston.md` |
| 16 | IYC (International Yacht Company) | Yacht Charter | 68 | PROFILED | `profiles/iyc.md` |
| 17 | Fraser Yachts | Yacht Charter | 68 | PROFILED | `profiles/fraser-yachts.md` |
| 18 | Quark Expeditions | Expedition | 65 | PROFILED | `profiles/quark-expeditions.md` |
| 19 | Northrop & Johnson | Yacht Charter | 62 | PROFILED | `profiles/northrop-and-johnson.md` |
| 20 | Ponant | Expedition/Cruise | 58 | PROFILED | `profiles/ponant.md` |
| 21 | Six Senses | Luxury Resort | 52 | PROFILED | `profiles/six-senses.md` |
| 22 | Nomad Tanzania | Safari/Lodge | **80** | PROFILED | `profiles/nomad-tanzania.md` |
| 23 | Legendary Expeditions | Safari/Lodge | **78** | PROFILED | `profiles/legendary-expeditions.md` |
| 24 | Asilia Africa | Safari/Lodge | 76 | PROFILED | `profiles/asilia-africa.md` |
| 25 | Morley Yachts | Yacht Charter | 72 | PROFILED | `profiles/morley-yachts.md` |
| 26 | Hill Robinson | Yacht Charter | 65 | PROFILED | `profiles/hill-robinson.md` |
| 27 | Camper & Nicholsons | Yacht Charter | 62 | PROFILED | `profiles/camper-and-nicholsons.md` |
| 28 | Natural Selection | Safari/Lodge | **81** | PROFILED | `profiles/natural-selection.md` |
| 29 | Ker & Downey Africa | Safari/Lodge | **79** | PROFILED | `profiles/ker-downey-africa.md` |
| 30 | Aurora Expeditions | Expedition/Cruise | 64 | PROFILED | `profiles/aurora-expeditions.md` |
| 31 | Lindblad Expeditions | Expedition/Cruise | 62 | PROFILED | `profiles/lindblad-expeditions.md` |
| 32 | Ahoy Club | Yacht Charter | 58 | PROFILED | `profiles/ahoy-club.md` |
| 33 | Expedition Terra | Adventure/Surf | 38 | PROFILED | `profiles/expedition-terra.md` |
| 34 | ROAR AFRICA | Safari/Luxury Travel | **78** | PROFILED | `profiles/roar-africa.md` |
| 35 | African Bush Camps | Safari/Lodge | 76 | PROFILED | `profiles/african-bush-camps.md` |
| 36 | Song Saa Private Island | Luxury Resort | 72 | PROFILED | `profiles/song-saa.md` |
| 37 | Yachtzoo | Yacht Charter | 60 | PROFILED | `profiles/yachtzoo.md` |
| 38 | CharterWorld | Yacht Charter | 55 | PROFILED | `profiles/charterworld.md` |
| 39 | Silversea Expeditions | Expedition/Cruise | 48 | PROFILED | `profiles/silversea-expeditions.md` |

## Queue — Phase 1 (100 operators, 20 per vertical)

### Yacht Charter (20)
| # | Operator | Priority | Status |
|---|----------|----------|--------|
| 1 | Burgess Yachts | HIGH | PROFILED (72) |
| 2 | Fraser Yachts | HIGH | PROFILED (68) |
| 3 | Northrop & Johnson | HIGH | PROFILED (62) |
| 4 | IYC (International Yacht Company) | HIGH | PROFILED (68) |
| 5 | Cecil Wright & Partners | HIGH | PROFILED (74) |
| 6 | Y.CO | MEDIUM | PROFILED (71) |
| 7 | Edmiston & Company | MEDIUM | PROFILED (69) |
| 8 | Morley Yachts | MEDIUM | PROFILED (72) |
| 9 | Camper & Nicholsons | MEDIUM | PROFILED (62) |
| 10 | Hill Robinson | MEDIUM | PROFILED (65) |
| 11 | Ahoy Club | MEDIUM | PROFILED (58) |
| 12 | CharterWorld | MEDIUM | PROFILED (55) |
| 13 | Yachtzoo | MEDIUM | PROFILED (60) |
| 14 | SuperYachtFan (charter) | LOW | QUEUED |
| 15 | Ocean Independence | MEDIUM | QUEUED |
| 16 | Denison Yachting | MEDIUM | QUEUED |
| 17 | Worth Avenue Yachts | MEDIUM | QUEUED |
| 18 | TWW Yachts | LOW | QUEUED |
| 19 | Bluewater Yachting | LOW | QUEUED |
| 20 | 4Yacht | LOW | QUEUED |

### Safari / Lodge (20)
| # | Operator | Priority | Status |
|---|----------|----------|--------|
| 1 | Singita | HIGH | PROFILED (82) |
| 2 | Wilderness Safaris | HIGH | PROFILED (76) |
| 3 | Great Plains Conservation | HIGH | PROFILED (83) |
| 4 | Angama Mara | HIGH | PROFILED (81) |
| 5 | Asilia Africa | MEDIUM | PROFILED (76) |
| 6 | Legendary Expeditions | MEDIUM | PROFILED (78) |
| 7 | Nomad Tanzania | MEDIUM | PROFILED (80) |
| 8 | Ker & Downey Africa | MEDIUM | PROFILED (79) |
| 9 | Natural Selection | MEDIUM | PROFILED (81) |
| 10 | Roar Africa | MEDIUM | PROFILED (78) |
| 11 | African Bush Camps | MEDIUM | PROFILED (76) |
| 12 | Sanctuary Retreats | MEDIUM | QUEUED |
| 13 | Volcanoes Safaris | MEDIUM | QUEUED |
| 14 | Robin Pope Safaris | MEDIUM | QUEUED |
| 15 | Gibbs Farm | LOW | QUEUED |
| 16 | Cottar's 1920s | LOW | QUEUED |
| 17 | Ol Donyo Lodge | LOW | QUEUED |
| 18 | Jack's Camp / Uncharted Africa | LOW | QUEUED |
| 19 | Lamai Serengeti | LOW | QUEUED |
| 20 | Bushmans Kloof | LOW | QUEUED |

### Expedition (20)
| # | Operator | Priority | Status |
|---|----------|----------|--------|
| 1 | Quark Expeditions | HIGH | PROFILED (65) |
| 2 | Pelorus | HIGH | PROFILED (85) |
| 3 | Cookson Adventures | HIGH | PROFILED (84) |
| 4 | Expedition Terra | LOW | PROFILED (38) |
| 5 | Ponant | HIGH | PROFILED (58) |
| 6 | Aurora Expeditions | MEDIUM | PROFILED (64) |
| 7 | Lindblad Expeditions | MEDIUM | PROFILED (62) |
| 8 | Silversea Expeditions | MEDIUM | PROFILED (48) |
| 9 | Hurtigruten Expeditions | MEDIUM | QUEUED |
| 10 | Seabourn Expedition | MEDIUM | QUEUED |
| 11 | Scenic Eclipse | MEDIUM | QUEUED |
| 12 | Swan Hellenic | MEDIUM | QUEUED |
| 13 | Aqua Expeditions | MEDIUM | QUEUED |
| 14 | Polar Latitudes | MEDIUM | QUEUED |
| 15 | One Ocean Expeditions | MEDIUM | QUEUED |
| 16 | Heritage Expeditions | LOW | QUEUED |
| 17 | Poseidon Expeditions | LOW | QUEUED |
| 18 | Oceanwide Expeditions | LOW | QUEUED |
| 19 | G Adventures (luxury tier) | LOW | QUEUED |
| 20 | Atlas Ocean Voyages | LOW | QUEUED |

### Luxury Resort / Lodge (20)
| # | Operator | Priority | Status |
|---|----------|----------|--------|
| 1 | Aman Resorts | HIGH | PROFILED (70) |
| 2 | Six Senses | HIGH | PROFILED (52) |
| 3 | Nihi Sumba | HIGH | PROFILED (79) |
| 4 | Song Saa | MEDIUM | PROFILED (72) |
| 5 | Explora Journeys | MEDIUM | QUEUED |
| 6 | Soneva | MEDIUM | QUEUED |
| 7 | The Brando | MEDIUM | QUEUED |
| 8 | Clayoquot Wilderness Lodge | MEDIUM | QUEUED |
| 9 | Deplar Farm (Eleven) | MEDIUM | QUEUED |
| 10 | Sheldon Chalet | MEDIUM | QUEUED |
| 11 | Longitude 131 | MEDIUM | QUEUED |
| 12 | Sal Salis | MEDIUM | QUEUED |
| 13 | Three Camel Lodge | LOW | QUEUED |
| 14 | Fogo Island Inn | LOW | QUEUED |
| 15 | Canopy & Stars (luxury tier) | LOW | QUEUED |
| 16 | Pikaia Lodge | LOW | QUEUED |
| 17 | Mashpi Lodge | LOW | QUEUED |
| 18 | Inkaterra | LOW | QUEUED |
| 19 | Tierra Hotels | LOW | QUEUED |
| 20 | Alto Atacama | LOW | QUEUED |

### Adventure / Wellness (20)
| # | Operator | Priority | Status |
|---|----------|----------|--------|
| 1 | Natural Habitat Adventures | HIGH | PROFILED (81) |
| 2 | Butterfield & Robinson | HIGH | PROFILED (77) |
| 3 | White Desert | HIGH | PROFILED (82) |
| 4 | Backroads | MEDIUM | QUEUED |
| 5 | Mountain Travel Sobek | MEDIUM | QUEUED |
| 6 | REI Adventures (luxury tier) | MEDIUM | QUEUED |
| 7 | Exodus Travels (premium) | MEDIUM | QUEUED |
| 8 | Intrepid Travel (premium) | MEDIUM | QUEUED |
| 9 | Kamalaya | MEDIUM | QUEUED |
| 10 | SHA Wellness Clinic | MEDIUM | QUEUED |
| 11 | Rancho La Puerta | MEDIUM | QUEUED |
| 12 | Canyon Ranch | MEDIUM | QUEUED |
| 13 | COMO Shambhala | MEDIUM | QUEUED |
| 14 | Chiva-Som | MEDIUM | QUEUED |
| 15 | Wilderness Scotland | LOW | QUEUED |
| 16 | Much Better Adventures | LOW | QUEUED |
| 17 | 50 Degrees North | LOW | QUEUED |
| 18 | Terres d'Aventure | LOW | QUEUED |
| 19 | World Expeditions | LOW | QUEUED |
| 20 | Peregrine Adventures | LOW | QUEUED |

## Queue Summary

| Vertical | Total Queued | HIGH Priority | Profiled | Remaining |
|----------|-------------|---------------|----------|-----------|
| Yacht Charter | 20 | 5 | 13 | 7 |
| Safari/Lodge | 20 | 4 | 11 | 9 |
| Expedition | 20 | 5 | 8 | 12 |
| Luxury Resort | 20 | 3 | 4 | 16 |
| Adventure/Wellness | 20 | 3 | 3 | 17 |
| **TOTAL** | **100** | **20** | **39** | **61** |

---

*Queue managed by Operator Scout agent. HIGH priority operators researched first.*
